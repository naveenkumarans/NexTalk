...
export default function JoinMeeting() {
  ...
  const appId = 484648545;
  const serverSecret = "7a7d8f88c9b6041540086fa494028cb0";
  const myMeeting = async (element: any) => {
    const kitToken = ZegoUIKitPrebuilt.generateKitTokenForTest(
      appId,
      serverSecret,
      params.id as string,
      user?.uid || generateMeetingID(), // Use user?.uid if it exists, otherwise generateMeetingId()
      user?.displayName || generateMeetingID() // Use user?.displayName if it exists, otherwise generateMeetingId()
    );
